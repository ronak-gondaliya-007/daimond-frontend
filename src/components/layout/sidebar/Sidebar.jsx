                ...
                {sections.map((section) => (
                    <div key={section.title} className="mb-[20px]">
                        ...
                        {section.items.map((item) => (
                            <div
                                key={item.name}
                                className={`flex items-center p-[10px] cursor-pointer transition-all duration-200 ease-in-out rounded-[12px] 
                            hover:bg-[#cecccc] hover:font-bold
                            ${isSelected(item.name) ? 'bg-[#cecccc] font-bold' : ''}
                            `}
